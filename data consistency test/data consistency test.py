...
def Linear(molecule_new,molecule):
    ...
    return pro_pre,list(sorted_JSC.values())[0],list(sorted_JSC.keys())[0:n]


df["unconsistent score"]=0
for i in range(1,len(df.index)+1):
    print(i)
    molecule_new = df.loc[i, [f"Group {j + 1}" for j in range(424)]].values
    property_new=df.loc[i, f"{property}"]
    property_pre,JSC_new,list_training=Linear(molecule_new, molecule)
    df.loc[i,"JSC"]=JSC_new
    df.loc[i,"error"]=abs((property_pre-property_new)/property_new)
    if (JSC_new>0.7 and abs((property_pre-property_new)/property_new)>0.1) or (JSC_new>0.9 and abs((property_pre-property_new)/property_new)>0.05):
        df.loc[i, "unconsistent score"] += 1
        for j in list_training:
            df.loc[j,"unconsistent score"]+=1
df.to_excel(fr"{path}\results\consistency test.xlsx")
